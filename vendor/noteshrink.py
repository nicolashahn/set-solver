...
def get_argument_parser(filenames=[]):
    ...
    parser.add_argument(
        "-e",
        dest="postprocess_ext",
        default="_post.png",
        help="filename suffix/extension for " "postprocessing command",
    )
...
their samples together into one large array.
    ...
    all_samples = [s[: int(round(float(s.shape[0]) / num_inputs))] for s in all_samples]
